...
# coding: utf-8
"""
First run the LLM_inference.py to prepare the data for this script
"""
...
plot_argumentation_framework = False # Default=False; plot the argumentation framework if true
...
comparison_path = f'results/LLM_{dataset_name.name}_{argumentation_type.name}_comparison_all.txt'
cnt = 0
# Define headers
            ...
            for diversity_threshold in ([0.1, 0.2, 0.3, 0.4] if hyperparameter_optimization else [diversity_threshold]):
                ...
                for model_type in [model_types.Synthetic_model]: # curr_model_types:
                    for synthetic_ratio in [1.0]: # synthetic_ratio_list:
                        ...
                        # encoding
                        _, test_df, encoded_test_df, encoder = load_logisitc_regression(X_train, X_test, y_train,
                                                                                            y_test, model_type,
                                                                                            class_label_col_name, train_model=False)
                            ...
                            for i in tqdm(range(len(test_df))):
                                if test_df.iloc[i, -1] == negative_class:
                                    ...
                                    if method == methods.Our:
                                        neighbourhood_sizes = [10, 50, 100, 150, 200]
